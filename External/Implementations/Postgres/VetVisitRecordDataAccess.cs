...
{
    public class PGVetVisitRecordDataAccess : IVetVisitRecordDataAccess
    {
        private NpgsqlDataSource pgDataSource;
        private readonly ILogger<PGVetVisitRecordDataAccess> _logger;
        private static string tableName = "vetvisitrecords";

        public PGVetVisitRecordDataAccess(IConfiguration config, ILogger<PGVetVisitRecordDataAccess> logger)
        {
        ...
        public List<VetVisitRecord> GetVetVisitRecordsByVehicleId(int vehicleId)
        {
            try
            {
                ...
                var results = new List<VetVisitRecord>();
                using (var ctext = pgDataSource.CreateCommand(cmd))
                {
                    ...
                    using (NpgsqlDataReader reader = ctext.ExecuteReader())
                        while (reader.Read())
                        {
                            VetVisitRecord record = JsonSerializer.Deserialize<VetVisitRecord>(reader["data"] as string);
                            results.Add(record);
                        }
                }
                return results;
            ...
            {
                _logger.LogError(ex.Message);
                return new List<VetVisitRecord>();
            }
        }
        ...
        public VetVisitRecord GetVetVisitRecordById(int vetVisitRecordId)
        {
            try
            {
                ...
                var result = new VetVisitRecord();
                using (var ctext = pgDataSource.CreateCommand(cmd))
                {
                    ctext.Parameters.AddWithValue("id", vetVisitRecordId);
                    using (NpgsqlDataReader reader = ctext.ExecuteReader())
                        while (reader.Read())
                        {
                            result = JsonSerializer.Deserialize<VetVisitRecord>(reader["data"] as string);
                        }
                }
                return result;
            }
        ...
        public bool SaveVetVisitRecord(VetVisitRecord vetVisitRecord)
        {
            try
            {
                if (vetVisitRecord.Id == default)
                {
                    string cmd = $"INSERT INTO app.{tableName} (vehicleId, data) VALUES(@vehicleId, CAST(@data AS jsonb)) RETURNING id";
                    using (var ctext = pgDataSource.CreateCommand(cmd))
                    {
                        ctext.Parameters.AddWithValue("vehicleId", vetVisitRecord.VehicleId);
                        ctext.Parameters.AddWithValue("data", "{}");
                        vetVisitRecord.Id = Convert.ToInt32(ctext.ExecuteScalar());
                        if (vetVisitRecord.Id != default)
                        {
                            string cmdU = $"UPDATE app.{tableName} SET data = CAST(@data AS jsonb) WHERE id = @id";
                            using (var ctextU = pgDataSource.CreateCommand(cmdU))
                            {
                                ctextU.Parameters.AddWithValue("id", vetVisitRecord.Id);
                                ctextU.Parameters.AddWithValue("data", JsonSerializer.Serialize(vetVisitRecord));
                                return ctextU.ExecuteNonQuery() > 0;
                            }
                        }
                        return vetVisitRecord.Id != default;
                    }
                }
                else
                {
                    ...
                    using (var ctext = pgDataSource.CreateCommand(cmd))
                    {
                        ctext.Parameters.AddWithValue("id", vetVisitRecord.Id);
                        ctext.Parameters.AddWithValue("data", JsonSerializer.Serialize(vetVisitRecord));
                        return ctext.ExecuteNonQuery() > 0;
                    }
                }
            }
